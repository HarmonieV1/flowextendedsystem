...
export const useStore = create((set, get) => ({
  ...
  prependKlines: (older) => set((s) => {
    // Merge older data before existing, deduplicate by timestamp
    const existing = new Set(s.klines.map(k => k.t))
    const fresh = older.filter(k => !existing.has(k.t))
    return { klines: [...fresh, ...s.klines] }
  }),
