...
async function fetchLiqZones(symbol, lastPx) {
  try {
    const [oiRes, fundRes] = await Promise.all([
      fetch(`https://fapi.binance.com/futures/data/openInterestHist?symbol=${symbol}&period=5m&limit=1`).catch(()=>null),
      fetch(`https://fapi.binance.com/fapi/v1/premiumIndex?symbol=${symbol}`).catch(()=>null),
    ])
    const oiData   = oiRes?.ok ? await oiRes.json().catch(()=>[]) : []
    const fundData = fundRes?.ok ? await fundRes.json().catch(()=>({})) : {}

    const oi        = parseFloat(oiData[0]?.sumOpenInterest || 0) || 50000
    const funding   = parseFloat(fundData.lastFundingRate || 0)
    const markPrice = parseFloat(fundData.markPrice || lastPx) || lastPx
    ...
    console.warn('liq fetch:', e.message)
    // Fallback: generate estimated zones from lastPx alone
    if (lastPx > 0) return generateFallbackZones(lastPx)
    return null
  }
}

function generateFallbackZones(px) {
  const zones = []
  for (const lev of [5, 10, 20, 50, 100]) {
    const pct = 1/lev
    zones.push({ price: px*(1-pct), side:'long', usd: 50000000/lev, leverage:lev })
    zones.push({ price: px*(1+pct), side:'short', usd: 45000000/lev, leverage:lev })
  }
  return { zones, markPrice: px, oi: 50000, funding: 0.0001 }
}
...
export function LiquidationMap() {
  ...
  const load = useCallback(async () => {
    if (!lastPx) return
    setLoading(true)
    let result = await fetchLiqZones(pair, lastPx)
    // Fallback garanti si fetch a échoué
    if (!result || !result.zones?.length) {
      result = generateFallbackZones(lastPx)
    }
    setData(result)
    setLoading(false)
  }, [pair, lastPx])
  ...
  useEffect(() => {
    ...
    zones.forEach(z => {
      ...
      if (isLong) {
        grad.addColorStop(0, 'rgba(140,198,63,.0)')
        grad.addColorStop(1, 'rgba(140,198,63,.7)')
      } else {
      ...
      if (z.leverage === 10 || z.leverage === 20) {
        ctx.fillStyle = isLong ? 'rgba(140,198,63,.8)' : 'rgba(255,59,92,.8)'
    ...
    ctx.fillStyle = 'rgba(140,198,63,.7)'
    ctx.textAlign = 'left'
