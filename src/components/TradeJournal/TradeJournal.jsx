...
import { hasApiKeys, getMyTrades } from '../../lib/bitunix'
...
export function TradeJournal() {
  ...
  useEffect(() => {
    ...
    load()
    // Sync with Bitunix API trades
    if (hasApiKeys()) {
      getMyTrades().then(data => {
        const apiTrades = Array.isArray(data) ? data : data?.tradeList || []
        if (apiTrades.length) {
          const existing = loadTrades()
          const existingIds = new Set(existing.map(t => t.id))
          const newTrades = apiTrades
            .filter(t => !existingIds.has('api_'+t.orderId))
            .map(t => {
              let dateStr
              try { const ts = Number(t.ctime); const d = new Date(ts < 1e12 ? ts*1000 : ts); dateStr = isNaN(d) ? new Date().toISOString() : d.toISOString() } catch { dateStr = new Date().toISOString() }
              return {
                id: 'api_'+(t.orderId||t.tradeId||Date.now()+Math.random()),
                date: dateStr,
                pair: (t.symbol||'').replace('USDT','/USDT'),
                side: (t.side||'').toLowerCase() === 'buy' ? 'buy' : 'sell',
                type: 'futures',
                entry: t.price||t.tradePrice||'',
                exit: null,
                qty: t.qty||t.tradeQty||'',
                pnl: parseFloat(t.profit||t.realizedPNL||0),
                fee: parseFloat(t.fee||0),
                status: 'closed',
                note: `${t.leverage||''}× · API sync`,
              }
            })
          if (newTrades.length) {
          ...
          return (
            <div key={t.id} className={`${styles.row} ${t.status==='open'?styles.rowOpen:''}`}>
              ...
              <div className={styles.rowActions}>
                <span className={styles.date}>{(() => { try { const d = new Date(t.date); return isNaN(d) ? '—' : d.toLocaleDateString('fr') } catch { return '—' } })()}</span>
                {t.status==='open' && (
