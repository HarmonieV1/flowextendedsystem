...
export function LiquidityHeatmap() {
  ...
  const draw = () => {
    ...
    allLevels.forEach((level, i) => {
      ...
      ctx.fillStyle = isBid ? 'rgba(140,198,63,0.04)' : 'rgba(255,59,92,0.04)'
      ctx.fillRect(0, y, W, rowH - 1)

      // Intensity bar with gradient
      const g = ctx.createLinearGradient(0, 0, barW, 0)
      if (isBid) {
        g.addColorStop(0, `rgba(140,198,63,${0.25 + intensity * 0.55})`)
        g.addColorStop(1, 'rgba(140,198,63,0.02)')
      } else {
      ...
      if (isWall) {
        ...
        ctx.shadowBlur = 6
        ctx.fillStyle = isBid ? 'rgba(140,198,63,0.12)' : 'rgba(255,59,92,0.12)'
        ctx.fillRect(0, y, barW, rowH - 1)
        ctx.shadowBlur = 0
        // Left border accent
        ctx.fillStyle = isBid ? 'rgba(140,198,63,0.9)' : 'rgba(255,59,92,0.9)'
      ...
      if (isWall && rowH >= 12) {
        ...
        ctx.fillStyle = isBid ? 'rgba(140,198,63,0.85)' : 'rgba(255,59,92,0.85)'
