...
function detectHarmonics(candles, swings) {
  ...
  // Deduplicate by pattern name, keep highest conf — only fresh & quality patterns
  const seen = {}
  return results
    .filter(r => r.conf >= 75) // Minimum quality threshold
    .filter(r => r.points.Di >= candles.length * 0.65) // D must be in recent 35% of data
...
function drawPattern(canvas, result, candles) {
  ...
  for (let i = idxStart; i <= idxEnd; i++) {
    ...
    // Wick
    ctx.strokeStyle = bull ? `rgba(140,198,63,${alpha})` : `rgba(255,59,92,${alpha})`
    ctx.lineWidth = 1
    ctx.beginPath(); ctx.moveTo(x, yh); ctx.lineTo(x, yl); ctx.stroke()

    // Body
    ctx.fillStyle = bull ? `rgba(140,198,63,${alpha})` : `rgba(255,59,92,${alpha})`
  ...
  // Fill XABCD zone (semi-transparent polygon)
  ...
  pts.forEach((pt, j) => {
    ...
    ctx.textAlign   = 'center'
    ctx.fillText(pt.l, x, y + 4.5)

    // Price tag with background
    const priceStr = fmtPx(pt.p)
    const tw = ctx.measureText(priceStr).width + 8
    const ty = above ? y - 20 : y + 24
    ctx.fillStyle = 'rgba(0,0,0,.7)'
    ctx.fillRect(x - tw/2, ty - 7, tw, 14)
    ctx.fillStyle   = 'rgba(255,255,255,.85)'
    ctx.font        = 'bold 9px monospace'
    ctx.fillText(priceStr, x, ty + 3)
  })
      ...
      {selected && (
        <div className={styles.chartWrap}>
          <div className={styles.chartTop}>
            <button onClick={()=>setSelected(null)} style={{padding:'4px 10px',border:'1px solid var(--brd)',borderRadius:5,background:'transparent',color:'var(--txt)',fontSize:11,cursor:'pointer',fontWeight:700,marginRight:8}}>← Retour</button>
