...
// ── Referral codes ────────────────────────────────────────────────────────────
// Drift: créer ton code sur app.drift.trade → Overview → Referral
// 35% des fees de chaque utilisateur référé, payé en USDC on-chain
// Builder Codes (V2): intégration SDK avec frais embarqués par ordre
const DRIFT_REF  = 'FXSEDGE'  // ← remplacer par ton vrai code Drift

const LEVERAGE_PRESETS = [2, 5, 10, 20, 50, 100]

// Drift markets (Solana)
const DRIFT_MARKETS = {
  ...
  ADAUSDT:'ADA-PERP', DOTUSDT:'DOT-PERP', SUIUSDT:'SUI-PERP',
  INJUSDT:'INJ-PERP', NEARUSDT:'NEAR-PERP', OPUSDT:'OP-PERP',
  WIFUSDT:'WIF-PERP', JUPUSDT:'JUP-PERP', TIAUSUST:'TIA-PERP',
}
...
export function Futures({ onOpenWallet }) {
  ...
  const buildGmxUrl = () => {
    ...
    return `https://app.gmx.io/#/trade?market=${market}&isLong=${side === 'long'}&ref=FXSEDGE`
  }
  ...
  const isGmxPair = !!GMX_MARKETS[pair]
    ...
    <div className={styles.wrap}>
        ...
        <div className={styles.platRow}>
          ...
          >
            ...
            <div>
              ...
              <span className={styles.platTag}>Solana · 100+ perps · 35% fees</span>
            </div>
          </button>
          <button
          ...
          >
            ...
            <div>
              ...
              <span className={styles.platTag}>Arbitrum · Non-custodial</span>
            </div>
          </button>
        </div>
      ...
      <div className={styles.strip} style={{
        ...
        {platform === 'drift' ? (
          <span className={styles.stripText}>
            ⚡ Drift · DEX Solana · 101× levier · No KYC · 35% des fees reversés sur ton code ref
          </span>
        ) : (
          <span className={styles.stripText}>
            🔵 GMX v2 · Arbitrum · Non-custodial · Wallet MetaMask · Min $10
          </span>
        )}
        {platform === 'drift' && !DRIFT_REF.includes('remplacer') && (
          <span className={styles.refBadge}>Ref: {DRIFT_REF}</span>
        )}
        {platform === 'drift' && DRIFT_REF === 'FXSEDGE' && (
          <a href="https://app.drift.trade" target="_blank" rel="noreferrer" className={styles.setupRef}>
            → Créer ton code ref Drift ↗
          </a>
        )}
      </div>
      ...
      <div className={styles.form}>
        ...
        <div className={styles.levBlock}>
          ...
          <input type="range" min="1" max={platform === 'drift' ? 101 : 50}
            value={leverage} onChange={e => setLeverage(+e.target.value)}
            className={styles.levSlider}
            style={{ background: `linear-gradient(to right,var(--grn) ${leverage / (platform === 'drift' ? 101 : 50) * 100}%,var(--bg3) ${leverage / (platform === 'drift' ? 101 : 50) * 100}%)` }}
          />
        ...
        {qty && parseFloat(qty) > 0 && (
          <div className={styles.summary}>
            ...
            <div className={styles.sumRow}><span>Marge</span><span>${fmt(posUSD, 2)}</span></div>
        ...
        <div className={styles.note}>
          {platform === 'drift'
            ? 'Drift Protocol · Solana · Phantom / Solflare wallet requis · No KYC'
            : 'GMX v2 · Arbitrum · MetaMask requis · Min $10 · 0.0002 ETH exec fee'}
        </div>

        {/* Drift setup banner */}
        {platform === 'drift' && (
          <div className={styles.driftBanner}>
            <div className={styles.driftBannerTitle}>💰 Programme Referral Drift</div>
            <div className={styles.driftBannerText}>
              Crée ton code ref sur <strong>app.drift.trade → Overview → Referral</strong><br/>
              Tu gagnes <strong>35% des fees</strong> de chaque utilisateur référé · Paiement USDC on-chain · Pas de plafond
            </div>
      ...
      <div className={styles.footer}>
        {platform === 'drift'
          ? '⚡ Drift Protocol · Solana · 35% referral fees · Builder Codes disponibles'
          : '🔵 GMX v2 · Arbitrum · Non-custodial · Ref FXSEDGE'}
      </div>
    </div>
  )
}
